...
const int NUM_MAIN_WORKER_THREADS = 2*NUM_THREADS_IN_CPU;
...
bsReturn ChudnovskyPiBS::bs_multithreaded(mpz_class a, mpz_class b, int threadCount, int depth) //clearly thread safe because nothing from outside the function is written to.
{
	...
	else
	{
		...
		if (threadCount > 0)
		{
			...
			futAm = std::async(std::launch::async, &ChudnovskyPiBS::bs_multithreaded, this, a, m, subThreadCountLeft, depth); 
		}
		else
		{
		...
		if (threadCount > 0)
		{
			...
			futMb = std::async(std::launch::async, &ChudnovskyPiBS::bs_multithreaded, this, m, b, subThreadCountRight, depth);
		}
		else
		{
			//std::ostringstream stream;
...
mpz_class ChudnovskyPiBS::calculatePi()
{
	...
	bsReturn BSResult = bs_multithreaded(0, N, TOTAL_THREAD_COUNT, 0);//bs(0, N); 
